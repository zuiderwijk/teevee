...
Search remains full-catalog. PR #173 adds finite channel-management intents, safe provider-independent channel aliases at the shared Search boundary, hidden-channel indication and an explicit add action. After owner acceptance #5819111859 of design spec #5819090800, the Mijn-zenders management surface uses two zones: visible rows have eye visibility plus handle-only drag/drop; hidden rows use eye-off and canonical/default order with no drag meaning. Drag keeps provisional order in presentation state and persists one arbitrary-index move only on a changed drop; accessibility keeps explicit one-step move actions. Management-local Search filters both zones and disables reorder while active. Changes persist immediately, there is no save/toast flow, and no bulk show/hide control is introduced. The exact 49-channel activation, provider mapping expansion, issue #167 orchestration/cron acceptance and TMDB paths are unchanged.
...
Merged PR #168 solved the original feed-wide XMLTV materialisation failure, but production proved that one Edge worker still cannot own the entire D-3..D+8 refresh. The post-#168 RTL4 four-hour path completes at ~1279–1284 ms CPU / ~12–14 MB and, after TMDB secret configuration, stored 5 programmes and resolved/persisted all 3 eligible Film/Series identities with zero provider/persistence failures. The required full `guide-horizon` v12 smoke then hit HTTP 546 at exactly **2000 ms CPU / ~40.7 MiB** after already committing 60 coverage rows (five days × twelve channels). That partial progress proves the monolithic request is not an all-or-nothing transaction.

A follow-up one-full-television-day/all-current-12-channel production smoke established the safe unit: **524 programmes**, zero diagnostics, TMDB 35 eligible / 33 resolved / 33 persisted, zero provider/persistence failures, **1145 ms CPU / ~34.9 MiB**, HTTP 200. PR #172 therefore leaves the PR #168 streaming parser untouched and changes only hosted orchestration ownership.

Proposed ADR 0012 records the candidate durable work-item axis as `provider source/feed × Amsterdam television-day × bounded channel group`. `guide-horizon` now plans a durable run and returns 202 instead of parsing the horizon itself. The database stores one common observation timestamp and explicit child source/day/group scope. HTTP child requests contain only opaque `jobId` + UUID attempt token; the Edge worker must claim the server-owned scope before constructing the provider. One child is globally in flight at a time.

Technical Lead review then found four correctness/operability blockers and one 49-channel fail-closed gap. Development hardened that candidate so partial/unattributed/no-safe/subset writes became terminal `incomplete` and parent aggregation exposed `incomplete`; its interim stale rule still treated an exact echoed `ignored-stale` scope as success, which Independent QA later tightened with the canonical proof described below. Distinct six-hour request keys are always persisted as separate queued runs rather than being silently coalesced into older active work, and `cron-token-unavailable` is durable on the affected run/job. The lease is now **8 minutes**, exceeding Supabase's current paid hosted worker maximum of 400 seconds by 80 seconds, so recovery cannot overlap a still-live prior worker. Attempts remain bounded and retries preserve the original observation, so ADR-0007 newer coverage still wins.

The SQL orchestration state machine is no longer supported only by string/fake-RPC tests. Normal CI starts disposable **PostgreSQL 17**, installs minimal Supabase-compatible `vault` / `net` / `cron` / `extensions` stubs, executes the **exact migration file**, and behaviorally proves duplicate same-token claim, stale-token claim/completion, lease expiry/requeue, retry exhaustion, global single-flight, sibling continuation, parent failed/incomplete recomputation, common observation ownership, distinct scheduled buckets, persisted missing-token state, service-role execution and anon/authenticated rejection.

Independent QA review #5306032707 then found one final Guide-authority hole: ADR 0007 may return `ignored-stale` for an entire multi-channel write when only one target channel has one newer overlapping coverage segment. Echoed attempted channel IDs therefore cannot prove complete newer authority. Development changed `ignored-stale` from direct success to `requires-canonical-proof`. Every planned job now persists its exact canonical channel IDs next to provider IDs; Edge fail-closes if the claimed canonical scope no longer matches source mappings. Guide completion performs the decisive proof inside Postgres: under the same sorted per-channel advisory transaction locks as the canonical writer, it requires every expected channel to cover the entire child `[from,to)` with no gaps using only `schedule_coverage.generated_at >= parent observed_at`. The proof and terminal job transition commit atomically. Same-observation coverage from an earlier attempt and newer coverage qualify; any missing/gapped/older channel produces durable `incomplete`, never `succeeded`.

The PostgreSQL 17 orchestration smoke now loads the **real canonical schedule-store migration first** and executes ADR-0007 writes for the QA regressions: A newer/B missing; partial newer overlap on A; an earlier same-observation full write followed by a newer partial segment and an ignored-stale retry; rollback protection for that newer segment; and disjoint channel groups. This preserves ADR 0007's conservative stale rejection while making orchestration completion strictly stronger than the stale-write signal.

A second Lead integration pass then caught three important gaps before QA. First, the initial orchestration draft ran TMDB inside each Guide child and therefore violated accepted ADR 0011, whose canonical horizon rule is all Guide writes before the first TMDB request. PR #172 now uses two phases: Guide children may stage only eligible identity evidence privately after a stored canonical write, while the dispatcher refuses all TMDB-capable work until canonical Guide work is terminal. A failed Guide run discards staging without TMDB; a non-throwing incomplete run may later enrich only a stored safe subset. Second, the durable run envelope was raised from 512 to **1024** and is directly tested at the approved worst case **49 × 12 × group size 1 = 588 jobs**. Third, the executable database smoke now runs on PostgreSQL 17 to match hosted production more closely.

The final Technical Lead re-review found one remaining false-success path in that deferred phase: `enrichExternalContent()` intentionally returns owner-level unavailable and per-item provider/persistence failures as values, while the worker still completed every non-throwing value as durable success and deleted staging. The candidate now classifies deferred outcomes explicitly. Only an available result with zero provider and persistence failures succeeds; semantic unresolved/ambiguous matcher decisions remain valid terminal work. Missing TMDB secret/owner unavailable, provider failures and persistence failures become retryable durable failures. Staging survives those retries, each attempt keeps the existing 20 s owner budget, and the existing max-three attempt policy remains bounded. Retry success clears staging; exhausted attempt three marks only the external-content lifecycle `failed` and then clears staging. Canonical Guide authority is unaffected throughout. Deterministic tests cover outcome classification, and the PostgreSQL 17 migration smoke behaviorally covers owner-unavailable retry→success, provider failure, persistence failure, max-attempt exhaustion, staging retention/cleanup and Guide-authority isolation.

The current NL topology may legitimately use all 12 existing channels in one group because that exact day shape is measured below the 2 s CPU ceiling. The value is intentionally source-owned operational configuration, not a product constant. Source validation now additionally requires every source-owned canonical channel to have exactly one provider mapping. The queued 49-channel target spans at least NL and BE XMLTV feeds, so future expansion can add source configs and split groups without changing auth, retry, freshness or observability semantics.

Production was inspected read-only through the connected Supabase project during Development: project `eokszvpityhtysbwdduy` is healthy, `epg-refresh` v12 is active, the six-hour cron still calls `teevee.enqueue_development_epg_refresh()`, and live `cron.job_run_details` confirms why durable orchestration is necessary: the 06:17 cron row says `succeeded` while `net._http_response` for its queued request is HTTP 546. Hosted migration history also contains `20260923211231_recover_programme_classification_siblings`, which is not present as a current repository migration file; PR #172 deliberately does not alter or reconcile that prior hosted-history drift.
...
Deterministic regressions cover XMLTV timestamps/timezone offsets, channel and programme tags split over tiny chunks, CDATA/entities, production date, complete categories, credits, episode numbers, live/repeat tri-state, malformed timestamps, requested channel/window filtering, boundary overlap, complete/gap/no-programme/multi-channel coverage, all-channel compatibility and deterministic ordering across different chunking. The resource regression is production-scale in row count: 184 channels and 40,311 programme blocks, of which 40,260 belong to irrelevant channels, 50 belong to the requested channel outside the window and one intersects the requested window. The scanner structurally observes all 40,311 blocks, parses timestamps for only the 51 requested-channel rows and fully materialises exactly one programme; its unread raw buffer remains a small fraction of the full synthetic document.

CPU self-review also removed a structural-scan hotspot before handoff. A first candidate searched too broadly inside the remaining network chunk; a local Node 22 synthetic benchmark at approximately 28.4 MB / 40,282 programme rows showed chunk-size-sensitive scan cost of roughly 0.21–3.1 s. The corrected monotonic, Unicode-index-safe block scanner reduced the same benchmark to roughly 0.07–0.10 s across 16 KiB–256 KiB chunk sizes, before provider-specific rich parsing/normalisation. This benchmark is directional development evidence, not a substitute for the mandatory hosted smoke after merge.
...
The TMDB client is dependency-free and server-only. It owns a 2.5 s per-attempt timeout, at most one safe retry, bounded 429 `Retry-After`, non-retryable 4xx/malformed responses, request cancellation, request-scope caching and bounded concurrency. One shared 20 s owner AbortSignal now bounds both TMDB HTTP work and the final external-content PostgREST persistence request. The production token is named `TMDB_API_READ_ACCESS_TOKEN` and never enters mobile/public contracts.

A new private `teevee.programme_external_content_references` table stores one row per canonical programme ID plus only source/media/external ID/high confidence/matcher version/freshness timestamps. It deliberately does **not** use a cascading FK: ADR 0007 normally delete/reinserts canonical programme rows and a cascade would erase valid forward-filled identity on every refresh. Instead, a before-write trigger rejects unknown programme owners and a deferred programme-side ownership trigger preserves an unchanged `id/channel/start/end/title` tuple across that delete/reinsert cycle while removing a true rekey/correction/delete before commit. The service-role write RPC still shares canonical channel advisory locks, requires exact current broadcast fields and same-observation authoritative coverage, and rejects newer coverage/reference state. Repeats may share one TMDB identity; stale/rekeyed/deleted broadcasts cannot retain or receive a dangling reference.
...
Independent QA then identified a trust-boundary bug in the generic TMDB collection parser: valid JSON with missing/null/wrong-shaped required arrays could collapse into ordinary empty evidence and therefore produce a deterministic unresolved/numbering-mismatch decision. Required movie/TV/person search results, season episodes, movie credits/crew/cast/alternative-title structures, Series aggregate credits/cast/alternative-title structures and person movie-credit crew are now schema-strict. A correct `[]` remains valid empty evidence; malformed required structures throw `TmdbRequestError('malformed')`, increment enrichment provider-failure accounting and emit no persistence decision for that work group, so a prior valid external reference cannot be cleared by upstream schema corruption.

The second Technical Lead re-review closed the remaining element-level hole in that same trust boundary. A correctly typed array can no longer hide malformed evidence such as `results:[{}]`, `episodes:[{}]`, `crew:[{}]`, `cast:[{}]` or alternative-title/person-credit items with missing matching-critical fields. Candidate IDs must be positive integers; people/title/job strings used by matching must be non-empty; episode numbers must be integers; malformed elements throw `TmdbRequestError('malformed')` instead of being filtered into semantic absence. Crew validation remains intentionally narrow: the role is required to decide whether an item matters, while director-specific name/content-ID fields are required only after the role is actually `Director`. An integration regression drives a real `TmdbRequestSession` malformed search element through enrichment and proves provider-failure accounting with zero persistence decision/write.

The third Technical Lead re-review identified the final scalar variant of the same trust-boundary issue: TMDB `release_date` was optional but its decoder previously collapsed wrong-shaped/non-date values into the same `null` as legitimate absence. The shared movie/director-filmography decoder now keeps missing, `null`, empty and whitespace-only release dates as unknown year while requiring every present non-empty value to be a valid calendar `YYYY-MM-DD` string. Non-string values, incomplete dates and impossible month/day combinations throw `TmdbRequestError('malformed')`. Film year thresholds are unchanged; this only prevents corrupted upstream year evidence from becoming deterministic unresolved evidence. Deterministic gateway tests cover optional absence and malformed values in both movie detail and director filmography, and a real-gateway enrichment regression proves malformed release date becomes provider failure with zero persistence decision/write.

Independent QA then found two remaining schema-strict edge cases on the corrected candidate. First, `seriesHasEpisode()` validated season entries inside short-circuiting `.some()`, so a matching valid episode could hide a malformed trailing element and turn schema-corrupt season data into positive S/E evidence. The request session now fully decodes every `episode_number` before membership is checked; regressions cover trailing-malformed, valid-present, valid-absent and existing HTTP-404 semantics, plus a real gateway/enrichment path proving the malformed season produces provider failure and no persistence write. Second, non-empty `release_date` strings are now validated in their original exact form: whitespace-only remains optional absence, but whitespace-padded otherwise-valid dates are malformed rather than normalized into matching evidence. These fixes do not change Film/Series thresholds, persistence/lifecycle, D0 policy, RPC ownership or the 20 s enrichment budget.

QA's non-blocking 20 s observation was also closed without a new RPC abstraction: the existing trusted PostgREST wrapper now accepts an optional owner AbortSignal, and only the external-content persistence client receives the same controller used by TMDB. The canonical schedule/auth RPC paths remain unchanged.

Self-review found that a naive FK cascade would conflict with ADR-0007 delete/reinsert retention, so the migration was corrected before handoff. The ownership helpers were then reduced to `SECURITY INVOKER` with empty `search_path` and public/anon/authenticated execution revoked. The final disposable PostgreSQL 17 lifecycle smoke passed on exact migration/smoke head `8c86fa7bd258d2c00d7341153652c4fc205daa72`: run `35931086447`, job `107417562049` — **SUCCESS**. Migration blob `3cd8bbc2d0e6a1a7bb4320d0bc4510f0b543650e` and smoke blob `d61d645293499c37b4c656193941c6df79818482` are the reviewed executable evidence. The smoke recreates Supabase's production permission semantics with `service_role ... BYPASSRLS`, executes the canonical schedule-store migrations plus the actual external-content migration, forces deferred ownership constraints at the decisive refresh/correction/delete points, and proves exact-broadcast refresh preservation, idempotent same-observation resolution, deterministic unresolved clearing, true correction/rekey cleanup, stale-late-result rejection, shared TMDB identity and authoritative deletion cleanup before rolling back. The temporary smoke workflow was removed from the final PR diff. No hosted migration, Edge deployment, merge or physical owner acceptance is claimed; exact-head CI, Technical Lead review and Independent QA remain required.
