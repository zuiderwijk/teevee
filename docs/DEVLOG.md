...
A disposable exact-implementation live probe then found an important defect before review: the first generic Series rule also admitted factual/panel formats (`The Yorkshire Vet`, `Sluipschutters`, `LUBACH`, `Beste Kijkers`, `Het Interventie Team`, `Top Gear`). Raw-source evidence showed why actor/cast presence cannot solve this: presenters are frequently encoded as actors. The provider boundary was tightened to carry only minimal director-credit presence and the classifier now gives category-format conflicts precedence. Final live probe run #4 / `35857949057` passed over 965 evening rows: 36 Film, 99 general/mainstream Series and 3 Sport inclusions. Its only generic adult-Series recoveries were exactly `The Spencer Sisters`, `Best Medicine`, `Missie Aarde`, `Agatha Christie's Poirot` and `Aspe`; Sport resolved to 2 events + 1 highlights, with talk/documentary excluded.
