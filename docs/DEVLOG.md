...
The first disposable PostgreSQL 17 lifecycle smoke exposed no SQL errors, but later self-review found that a naive FK cascade would conflict with ADR-0007 delete/reinsert retention. The migration and smoke were corrected to prove exact-broadcast preservation plus true rekey/delete cleanup; fresh PostgreSQL smoke and exact-head CI are required on that corrected lineage before handoff. No hosted migration, Edge deployment, merge or physical owner acceptance is claimed.
