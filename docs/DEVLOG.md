...
Self-review found that a naive FK cascade would conflict with ADR-0007 delete/reinsert retention, so the migration was corrected before handoff. The corrected disposable PostgreSQL 17 lifecycle smoke passed on head `bde9bbd759f9df50b4aed0b87f1fab41570806a2`: workflow run #5 / `35928698305`, job `107409866483` — **SUCCESS**. It executed the canonical schedule-store migrations plus the actual external-content migration and proved exact-broadcast refresh preservation, idempotent same-observation resolution, deterministic unresolved clearing, true correction/rekey cleanup, stale-late-result rejection, shared TMDB identity and authoritative deletion cleanup, then rolled back. The temporary smoke workflow is not part of the final PR diff. No hosted migration, Edge deployment, merge or physical owner acceptance is claimed; exact-head CI, Technical Lead review and Independent QA remain required.
