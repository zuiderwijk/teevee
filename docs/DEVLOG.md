...
Self-review found that a naive FK cascade would conflict with ADR-0007 delete/reinsert retention, so the migration was corrected before handoff. The ownership helpers were then reduced to `SECURITY INVOKER` with empty `search_path` and public/anon/authenticated execution revoked. The final disposable PostgreSQL 17 lifecycle smoke passed on exact migration head `26c11f74303a436b9ab25eafdd0b87bdd17123e3`: run `35928893022`, job `107410501655` — **SUCCESS**. Migration blob `3cd8bbc2d0e6a1a7bb4320d0bc4510f0b543650e` and smoke blob `23f8792fc34f8fee3437d2794e721ef91d788408` are unchanged in the final candidate. The smoke executed the canonical schedule-store migrations plus the actual external-content migration and proved exact-broadcast refresh preservation, idempotent same-observation resolution, deterministic unresolved clearing, true correction/rekey cleanup, stale-late-result rejection, shared TMDB identity and authoritative deletion cleanup, then rolled back. The temporary smoke workflow was removed from the final PR diff. No hosted migration, Edge deployment, merge or physical owner acceptance is claimed; exact-head CI, Technical Lead review and Independent QA remain required.
