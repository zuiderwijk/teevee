...
The Kijktip label surface uses production-frozen **`editorialAccentSurface`**: light **#EEECE7**, dark **#171715**. Foreground contrast on this surface is **6.41:1 light / 10.21:1 dark**. Surface separation from the Guide canvas is intentionally subtle at about **1.10:1 light / 1.06:1 dark**. Never use a saturated petrol fill as the surface. The label has no border, shadow/elevation or independent pressed state; parent programme pressed feedback remains authoritative.
...
In **Per zender**, the label is one compact, more-square unit in the existing time-column zone containing both **start time + Kijktip**. Both texts use `editorialAccent`; the box uses `editorialAccentSurface`. **X24 is the structural start-time text origin for every row**; with fixed 8-pt horizontal inset the surface begins at X16. Radius6, min width56 and width=max(56,max(intrinsic time,intrinsic Kijktip)+16) remain unchanged. At S1 the standard surface fills the existing 52-pt row and uses 7-pt top breathing + 20-pt time + fixed 2-pt gap + 16-pt Kijktip + 7-pt bottom breathing. At Larger Text that breathing is derived from the existing scaled standard-row height, never from a Kijktip-specific row branch. Current uses the same internal breathing with its surface anchored at the existing current-content top14; current title/description/progress remain untouched. The earlier surface-left X24 / vertical-padding0 rule is superseded.
