...
In **Per zender**, the label is one compact, more-square unit in the existing time-column zone containing both **start time + Kijktip**. Both texts use `editorialAccent`; the box uses `editorialAccentSurface`. **X24 is the structural start-time text origin for every row**; fixed 5-pt horizontal padding puts the compact surface at X19. Radius6, min width48 and width=max(48,max(intrinsic time,intrinsic Kijktip)+10). At S1, accepted content positions remain time Y7…27 and Kijktip Y29…45 while the surface is Y2…50, giving 5/20/2/16/5 inside the surface and 2 pt external row breathing. At Larger Text the surface is 36S+12 inside the unchanged standard-row authority; no Kijktip-specific height branch exists. Current keeps time Y21…41/Kijktip Y43…59 and uses surface Y16…64 at S1; current title/description/progress remain untouched. The earlier surface-left X24 / vertical-padding0 rule is superseded.
