...
- issue #167 has two distinct evidence phases: pre-#168 full-horizon and RTL4-only requests both failed at ~2.15 s CPU because feed-wide XMLTV materialisation ignored requested bounds; merged PR #168 reduced the narrow RTL4 path to ~1.28 s CPU / ~12–14 MB and successfully activated TMDB persistence, but the v12 monolithic full horizon still hit exactly 2000 ms CPU after partially committing five days. One complete current 12-channel television day then passed at 1145 ms CPU / ~34.9 MiB including canonical write and TMDB, establishing bounded multi-invocation orchestration as the required residual correction.

### Automatic development refresh
A server-side `pg_cron` + `pg_net` job runs every six hours. The protected `guide-horizon` request is an **orchestration command**, not one all-horizon ingest transaction. The Edge planner derives D-3..D+8 from the canonical 06:00 Europe/Amsterdam television-day primitive and persists a durable run whose Guide children are keyed by:

`provider source/feed × Amsterdam television-day window × bounded provider-channel group`

Merged PR #172 / canonical ADR 0012 owns this topology. Every run carries one `observed_at`; every Guide child uses that exact timestamp for ADR-0007 freshness/stale-write semantics. Postgres, not the HTTP caller, owns each source/window/channel set. A dispatched child receives only `jobId + attemptToken`, claims the real scope through a service-role RPC, performs one existing bounded streaming XMLTV ingest and records a durable terminal outcome.

Child terminal authority is exact-scope. A `stored` result is directly `succeeded` only for the whole expected canonical channel group. `ignored-stale` is **never** authority by itself: each job persists its exact canonical channel IDs, and Guide completion asks Postgres to verify the whole `[from,to)` scope atomically under the same sorted channel advisory locks as ADR 0007. Every expected channel must have gap-free canonical coverage with `generated_at >= parent observed_at`; same-observation coverage from an earlier attempt and newer coverage both qualify. Missing/gapped/older scope becomes terminal `incomplete`. Provider partial coverage, unattributed/no-safe data and channel-local blocked subsets remain terminal `incomplete`; a parent with an incomplete child and no exhausted failure ends `incomplete`, not `completed`. An exhausted Guide retry produces `failed`.

ADR 0011's horizon sequencing is preserved with a second, private lifecycle instead of inline TMDB per child. A successfully stored observation may lifecycle-stage only eligible current/future Film/Series provider evidence in the private orchestration row. **No external-content work item is dispatched while any canonical Guide job is queued/dispatched/running.** After Guide processing is terminal, deferred `external-content-work-item` children claim only database-owned staged evidence. A failed Guide run marks staged identity work `skipped` and deletes it without TMDB; a non-throwing incomplete run may enrich only its actually stored safe subset.

Deferred lifecycle success is now explicit: only an available enrichment result with **zero provider failures and zero persistence failures** becomes `completed`. Semantic unresolved/ambiguous decisions are valid successful matcher outcomes. Missing TMDB secret/owner-level unavailable, provider failures and persistence failures are retryable via the existing lease/attempt machinery; staging is retained across retries, the 20 s owner budget applies independently to every attempt, and attempts remain bounded at three. A later success clears staging; max-attempt exhaustion marks external-content durably `failed` and then clears staging. These states are separate from Guide authority, so TMDB/persistence failure never rolls back, downgrades or blocks canonical Guide state. Staging remains lifecycle-only, not canonical content/history/cache and never public/mobile. Matcher thresholds and broadcast-keyed persistence are unchanged.

Current concurrency is deliberately **one hosted child globally across both phases**. Guide work has dispatch priority. Distinct six-hour request keys are persisted as distinct runs even while older work is active. Missing Vault dispatch credentials are persisted rather than disappearing in transient SQL output. Guide and enrichment attempts use an **8-minute lease**, deliberately above Supabase's current 400-second paid hosted worker maximum, so recovery cannot replace a child while its old hosted worker can still be alive.

The durable planner/database envelope is **1..1024 Guide work items per run**. The approved 49-channel catalog fits even at group size one: 49 channels × 12 D-3..D+8 television days = **588 jobs**. The current NL source may still use an operational group maximum of 12 channels because that exact one-day/all-12 production smoke measured 1145 ms CPU with persistence/TMDB. Group size is not a catalog contract. Source validation requires every source-owned canonical channel to have exactly one provider mapping and rejects duplicate source/provider/canonical ownership, so adding the NL+BE 49-channel topology remains configuration/measurement work rather than an orchestration rewrite.

For Issue #175 / PR #176, the concrete 49-channel candidate topology is explicit but **not production-activated**:

- NL source owns 36 canonical channels; BE source owns 13;
- provider IDs are explicit adapter mappings; renamed brands keep provider vocabulary such as `FOX.nl`, `BBCFirst.nl` and `Play4.be`–`Play7.be` without leaking those IDs into mobile state;
- current candidate max group size is 12 for each source, yielding 5 Guide children per television day / 60 for the 12-day backend envelope;
- the 588-job group-size-one case remains the fail-safe envelope proof;
- **12 is not capacity evidence for the expanded feeds**. Before production activation, hosted source-specific smoke must prove the heaviest NL group and BE group across a complete Amsterdam television day, including programme count, diagnostics, CPU, memory, HTTP status and completeness. If headroom is insufficient, lower the source-specific group size rather than weakening parser/completeness/retry semantics;
- the temporary development XMLTV source still does not reliably prove the complete product D-2..D+7 horizon; ADR 0007/0008 fail-closed and production provider/rights/SLA gates remain unchanged.


The merged PR #168 incremental parser remains canonical inside each Guide child: requested provider channels and the exact day window constrain rich programme materialisation before normalisation/classification/persistence. Each child independently preserves `complete | partial` provider authority; partial work never destructively replaces canonical coverage. The exact orchestration migration is executed behaviorally in normal CI on disposable **PostgreSQL 17** together with the real canonical schedule-store migration. Coverage includes service-role/anon/authenticated boundaries, duplicate/stale attempts, lease recovery, distinct scheduled buckets, mixed-channel ADR-0007 stale rejection, partial newer overlap, same-observation retry authority, stale rollback safety, disjoint-group isolation, Guide authority aggregation, Guide-before-TMDB sequencing, owner/provider/persistence external-content retries, retry-to-success, max-attempt staging cleanup, Guide-authority isolation and the 588-job boundary. D-2..D+7 remains the selectable product horizon and D-3/D+8 remain backend safety buffers only.

Production activation is **closed and proven**. PR #172 merged as `f66c3967d81a76113e8c29a0e9e6ffb58bae105d`; exact-main CI #1375 / run `36023646968` passed. Production migration history is reconciled to canonical versions `20260924001500_create_programme_external_content_reference` and `20260924095903_create_epg_refresh_orchestration`; the earlier mis-timestamped hosted records were repaired as migration-history tracking only, without replaying SQL. `epg-refresh` v13 is ACTIVE. A protected rollout run and the first normal six-hour cron both planned exactly 12 television-day Guide children. The normal cron at 2026-09-24 18:17 UTC completed with 5 source-complete children succeeded, 7 source-partial children correctly terminal `incomplete`, 0 failed Guide jobs, 0 HTTP 546 responses and 0 CPUTime shutdowns; max observed worker CPU was 1282 ms and max memory ~14.8 MiB. Deferred external-content completed all 5 applicable jobs after Guide terminal, with no provider/persistence failures, no staged observations remaining, 160 references and 0 dangling references. The temporary development feed's missing D-2/D-1 and partial D+6/D+7 coverage remains a provider-horizon limitation, not an orchestration defect.
