...
A server-side `pg_cron` + `pg_net` job runs every six hours. The protected `guide-horizon` request is an **orchestration command**, not one all-horizon ingest transaction. The Edge planner derives D-3..D+8 from the canonical 06:00 Europe/Amsterdam television-day primitive and persists a durable run whose children are keyed by:

`provider source/feed × Amsterdam television-day window × bounded provider-channel group`

PR #172 / proposed ADR 0012 owns this candidate topology until review/merge makes it canonical. Every run carries one `observed_at`; every child uses that exact timestamp for ADR-0007 freshness/stale-write semantics. Postgres, not the HTTP caller, owns each child's source/window/channel set. A dispatched child receives only `jobId + attemptToken`, claims the real scope through a service-role RPC, performs one existing bounded streaming XMLTV ingest plus fail-open external-content enrichment, then records a durable terminal outcome. Duplicate delivery is idempotent and stale attempt tokens cannot claim/complete a newer lease. The lease is **8 minutes**, deliberately above Supabase's current 400-second paid hosted worker maximum; the recovery pump therefore cannot replace a child while its previous hosted worker can still be alive. Cron enqueue success is not refresh success: durable run/job terminal state is the operational authority.

Child terminal authority is exact-scope. `stored` or `ignored-stale` counts as `succeeded` only when the write covers the whole expected canonical channel group. Provider partial coverage, unattributed/no-safe data and channel-local blocked subsets are terminal `incomplete`; a parent with any incomplete child and no exhausted failure ends `incomplete`, not `completed`. An exhausted retry produces `failed`. A safely stored subset may still feed fail-open external-content enrichment, but does not upgrade the enclosing child to authoritative success.

Current concurrency is deliberately **one child globally**. Any terminal child permits the dispatcher to continue with the next available job; a one-minute recovery pump exists only to recover expired/queued retry work. Distinct six-hour request keys are persisted as distinct runs even while older work is active, so scheduled buckets cannot disappear through coalescing; missing Vault dispatch credentials are persisted on the queued run/job. This protects the XMLTV source, TMDB fan-out and canonical database writes from accidental burst concurrency while preserving operational history.

The current NL source has an operational group maximum of 12 channels because the exact one-day/all-12 production smoke measured 1145 ms CPU with full persistence/TMDB. That number is not a product/catalog contract. Source validation requires every source-owned canonical channel to have exactly one provider mapping and rejects duplicate source/provider/canonical ownership. The owner-approved 49-channel expansion may therefore lower group size and add `epg-be.xml` as another source without an orchestration rewrite.

The merged PR #168 incremental parser remains canonical inside each child: requested provider channels and the exact day window constrain rich programme materialisation before normalisation/classification/persistence. Each child independently preserves `complete | partial` provider authority; partial work never destructively replaces canonical coverage. The exact orchestration migration is also executed behaviorally in CI on disposable PostgreSQL 16 rather than being trusted through string tests alone. D-2..D+7 remains the selectable product horizon and D-3/D+8 remain backend safety buffers only.
