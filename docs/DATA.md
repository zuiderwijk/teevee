...
The merged PR #168 incremental parser remains canonical inside each Guide child: requested provider channels and the exact day window constrain rich programme materialisation before normalisation/classification/persistence. Each child independently preserves `complete | partial` provider authority; partial work never destructively replaces canonical coverage. The exact orchestration migration is executed behaviorally in normal CI on disposable **PostgreSQL 17**, including service-role/anon/authenticated boundaries, duplicate/stale attempts, lease recovery, distinct scheduled buckets, Guide authority aggregation, Guide-before-TMDB sequencing, failed-run suppression, staging cleanup and the 588-job boundary. D-2..D+7 remains the selectable product horizon and D-3/D+8 remain backend safety buffers only.
