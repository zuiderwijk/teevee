...
Owner-approved production specification: `docs/TOTAAL_VISUAL_CONVERGENCE.md` — canonical Development authority on merge to `main`
...
- Per-zender time grid is authoritative: **X24 is the start-time text origin for every row**. Compact horizontal padding5 puts the editorial surface at **X19**; radius6/min-width48/width=max(48,max(intrinsic time,intrinsic Kijktip)+10). Standard S1 content positions stay time Y7…27 / Kijktip Y29…45 while the surface is Y2…50 (5/20/2/16/5 internally plus 2-pt external row breathing). Current content positions remain time Y21…41 / Kijktip Y43…59 with surface Y16…64; title/description/progress do not move. At Larger Text surface height is 36S+12 inside the unchanged row formulas;
...
The exact date choices in the original 13 September Per-zender image are **SUPERSEDED**. Any selected-channel text outside the persistent logo rail, the old 24-pt rail→utilities gap, the physically rejected 16/12 expanded spacing distribution, the long `Vandaag/Morgen · weekday date` labels, relative labels between 00:00–05:59, Dynamic-Type-driven wrapped temporal chrome and the earlier compact 120-pt current-programme treatment are likewise superseded by the written 18 September refinements. The earlier 22 September text-only Kijktip board is superseded for final Kijktip styling by the owner-approved shared time+Kijktip editorial label calibration. The written production metrics are authoritative; later physical runtime validation is an implementation-fidelity gate, not an open design decision. Preserve the rest of the accepted channel-strip/list composition.
