...
Therefore the residual correction is orchestration, not another parser rewrite. PR #172 / proposed ADR 0012 makes `guide-horizon` a lightweight durable planner and executes bounded Guide children keyed by source/feed × 06:00 Amsterdam television day × configurable channel group. One common observation timestamp preserves ADR-0007 freshness semantics; each child claims database-owned scope, retries under an 8-minute lease/attempt token and records durable exact-scope authority.

Technical Lead hardening makes orchestration completion exact-scope: only a whole-group `stored` or exact-scope `ignored-stale` child is `succeeded`; partial provider coverage, unattributed/no-safe data or a blocked canonical subset is `incomplete`. Distinct scheduled buckets are independently durable, missing Vault dispatch credentials are observable, complete mapping validation fails closed, and the lease exceeds the current 400-second paid hosted worker maximum so recovery cannot overlap a live old attempt.

A second Lead integration pass preserved accepted ADR 0011 explicitly. Guide children no longer call TMDB. A stored observation may lifecycle-stage only its eligible current/future Film/Series tuple in private orchestration state; the dispatcher releases no `external-content-work-item` while canonical Guide work remains non-terminal. A failed Guide run deletes staged evidence without TMDB, while a non-throwing incomplete run may later enrich only a safely stored subset. Terminal enrichment clears staging. Matching/persistence thresholds and the 20 s owner budget are unchanged.

The durable run envelope is now **1024** work items. The approved 49-channel topology fits at the worst-case group size one (**49 × 12 = 588 jobs**) and that exact boundary is exercised in planner and migration lifecycle tests. The current 12-channel NL group remains only a measured operational setting. The exact migration lifecycle is executed in normal CI against disposable **PostgreSQL 17**, including service-role/anon/authenticated boundaries, retries/leases, distinct buckets, Guide authority aggregation, Guide-before-TMDB sequencing, failed-run suppression, evidence cleanup and the 588-job case.

Deployment remains gated. During PR #172 Development the connected Supabase project is inspected read-only only: no migration, cron mutation or Edge deployment is performed. After merge/exact-main CI, apply the reviewed orchestration migration first (safe with existing v12 because it does not replace the six-hour enqueue function), deploy exact merged `epg-refresh`, trigger one protected guide-horizon run and require every expected Guide child to reach the correct terminal authority; any `incomplete`/ `failed` child is a rollout failure to diagnose, not a successful horizon. Verify that no TMDB child starts before Guide completion, that deferred external-content state reaches terminal with staging cleared, then verify coverage/references/orphans and require the next normal six-hour cron run as independent proof.
