...
Therefore the residual correction is orchestration, not another parser rewrite. PR #172 / proposed ADR 0012 makes `guide-horizon` a lightweight durable planner and executes bounded children keyed by source/feed × 06:00 Amsterdam television day × configurable channel group. One common observation timestamp preserves ADR-0007 freshness semantics; each child claims database-owned scope, retries under a lease/attempt token, records durable outcome, and runs existing fail-open TMDB enrichment after canonical write.

Technical Lead hardening makes orchestration completion exact-scope: only a whole-group `stored` or exact-scope `ignored-stale` child is `succeeded`; partial provider coverage, unattributed/no-safe data or a blocked canonical subset is `incomplete`, so the parent cannot masquerade as fully completed. Distinct scheduled buckets are persisted independently, missing Vault dispatch credentials are durably visible, and the 8-minute lease exceeds the current 400-second paid hosted Edge-worker maximum so recovery cannot overlap a still-live old attempt. Current child concurrency is one. The current 12-channel NL group remains a measured operational setting, not a catalog invariant; complete mapping validation plus configurable source/group partitioning preserves the owner-approved 49-channel NL+BE path. The exact migration lifecycle is executed in CI against disposable PostgreSQL 16 with service-role and anon/authenticated boundary checks.

Deployment remains gated. During PR #172 Development the connected Supabase project is inspected read-only only: no migration, cron mutation or Edge deployment is performed. After merge/exact-main CI, apply the reviewed orchestration migration first (safe with existing v12 because it does not replace the six-hour enqueue function), deploy exact merged `epg-refresh`, trigger one protected guide-horizon run and require parent `completed` with every expected source/day/group child authoritatively succeeded; any `incomplete`/ `failed` child is a rollout failure to diagnose, not a successful horizon. Then verify coverage/external-content/orphans and require the next normal six-hour cron run as independent proof.
