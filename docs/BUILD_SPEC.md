...
8. **IN DEVELOPMENT — PR #144 / issue #142** — implement the central provider-independent Film/Series/Sport classification/enrichment foundation required by the empirical research: full provider evidence preserved server-side, deterministic fail-closed mapping, ingest-owned sibling persistence/lifecycle and a separate bounded semantic read boundary; no production Vanavond category UI in this increment;
