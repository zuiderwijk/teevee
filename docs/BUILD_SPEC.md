...
8. **DEVELOPMENT COMPLETE / REVIEW PENDING — PR #144 / issue #142** — central provider-independent Film/Series/Sport classification/enrichment foundation: classification-relevant provider evidence (full categories, episode numbers and minimal director-credit presence) stays server-side, deterministic fail-closed mapping has been live-probed against the mapped source, ingest-owned sibling persistence/lifecycle is PostgreSQL-smoke-proven, and a separate bounded semantic read boundary keeps Guide loading unchanged; Lead + Independent QA + hosted deployment remain required before production Vanavond category runtime;
9. only after that foundation is authoritative and, where applicable, deployed/live-verified, implement the production Vanavond runtime against the accepted visual/product contracts; artwork enrichment remains optional and must stay provider-independent/rights-cleared.
