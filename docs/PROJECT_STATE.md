...
- refine the canonical Nu & Straks visual/accessibility contract from the physical iPhone evidence on PR #96 exact head `32db9459d265e8546c7137baf82e623f015ad652`: rail-tick legibility, large-text composition and perceived density of the three following programmes;
- after owner acceptance, merge the refined design/spec baseline to `main` and reconcile the existing PR #96 runtime again;
...
## EXACT NEXT STEP
**Return Nu & Straks to targeted Design / UX refinement using the physical iPhone evidence from PR #96 exact head `32db9459d265e8546c7137baf82e623f015ad652`. Dark mode as a whole is accepted, but physical acceptance is NOT granted. Refine only: (1) rail positional tick contrast/semantic colour so major and quarter hairlines remain subtle but legible, especially in dark mode; (2) large-text/Dynamic Type composition so Guide chrome remains understandable and substantive programme text remains accessible without the current excessive vertical expansion; and (3) perceived density of the three following programmes by separating visual text rhythm from the required independent 44-pt iOS / 48-dp Android interaction geometry. Do not shrink or overlap touch targets and do not reintroduce cards, artwork, genres, progress bars or extra labels. After owner acceptance, merge the refined canonical spec before reconciling the existing runtime PR #96 again. Do not resume physical validation, request Independent QA or merge PR #96 before that reconciliation.**

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/ENGINEERING_QUALITY_POLICY.md`, ADR 0007, ADR 0008 and the relevant accepted visual handoff before changing the repository. Phase 1A, Phase 1B, Phase 2 and Phase 3 are closed on iPhone. Phase 4 is active. Per-zender production convergence is merged and closed through PR #88/#89. Programme Detail production convergence is merged and closed through PR #92. Nu & Straks production design is canonical through PR #93 and owner refinement PR #97. Runtime PR #96 was reconciled to that baseline and passed exact-head CI, but physical iPhone review of exact head `32db9459d265e8546c7137baf82e623f015ad652` was NOT accepted because rail hairlines are insufficiently legible, larger-text composition is not production-ready and the three following programmes still read too loose. The next step is targeted Design / UX refinement and canonical spec update; only then reconcile PR #96 again and restart physical validation.