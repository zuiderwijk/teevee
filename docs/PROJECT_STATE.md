...
## EXACT NEXT STEP
**Freeze the single remaining Programme Detail product decision in `docs/PROGRAMME_DETAIL_VISUAL_CONVERGENCE.md`: the default `Herinner mij` notification fire time. Choose either at programme start or 5 minutes before; configurable lead time remains out of Phase 4 scope. After that decision is recorded and the production specification is merged, implement Programme Detail as one separate HIGH-risk Phase 4 increment while preserving the mounted-Guide/modal boundary, Guide round-trip context, accessibility, themes and existing accepted Guide behaviour.**

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/ENGINEERING_QUALITY_POLICY.md`, ADR 0007, ADR 0008 and the relevant accepted visual handoff before changing the repository. Phase 1A, Phase 1B, Phase 2 and Phase 3 are closed on iPhone. Phase 4 is active. Per-zender production convergence is merged and closed through PR #88/#89: preserve its 52/176 fixed rows, 4/52/24 expanded temporal composition, 0/52/0 condensed composition, semantic anchors, channel identity stability, safe-area handling, selected-channel schedule-height ownership, gestures, D-2..D+7 and the frozen PR #81 fixed-native-viewport/collapse-isolation architecture. The Programme Detail production handoff now lives in `docs/PROGRAMME_DETAIL_VISUAL_CONVERGENCE.md`; all implementation details are specified except the owner decision for default `Herinner mij` notification timing. Do not start reminder runtime work until that single product decision is frozen canonically.