...
For Totaal, the **production visual design and implementation specification are canonical**. Canonical composition is `/Teevee/Totaal production design - light dark expanded condensed.png` (`file_00000000aafc8230b3218e6577a1d912`), the concise manifest is `design/current/guide/TOTAAL.md`, and the single Development handoff is `docs/TOTAAL_VISUAL_CONVERGENCE.md`. Frozen interaction/data constraints remain intact: 2D time/channel navigation, exact real programme geometry, shared day selector + `Nu`, native inertia/bounce/directional lock, Programme Detail context, Instrument Sans and the proven performance/windowing architecture.
...
## EXACT NEXT STEP
**Development — Totaal production visual convergence. Implement the canonical 21 September design exactly from `docs/TOTAAL_VISUAL_CONVERGENCE.md` on the existing proven Totaal 2D/windowing architecture. Preserve D-2..D+7, 06:00 television-day semantics, native inertia/bounce/directional lock, partial-left readability, programme-window ownership and Programme Detail round-trip. Converge only the specified shared Guide shell/collapse, day+Nu context, 44-pt time axis, logo-first rail, open programme grid, exact programme typography/current-marker treatment, Dynamic Type/accessibility and documented loading/edge states. This is HIGH risk: deterministic tests → exact-head CI → physical iPhone validation → Independent QA → Lead merge decision. Do not start Phase 5 Search before this increment closes.**

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/PRODUCT.md`, `docs/UX.md`, `docs/VISUAL_BASELINE.md`, `design/current/guide/TOTAAL.md`, `docs/TOTAAL_VISUAL_CONVERGENCE.md`, `docs/ARCHITECTURE.md`, `docs/DATA.md`, `docs/ENGINEERING_QUALITY_POLICY.md` and the relevant ADRs before changing the repository. Phase 4 remains ACTIVE. Per-zender, Programme Detail and Nu & Straks production convergence are merged and accepted on iPhone. Totaal interaction mechanics, television-day semantics and performance hardening are proven; its final production visual design and exact production specification are canonical. **Next step: Development implements Totaal production visual convergence against `docs/TOTAAL_VISUAL_CONVERGENCE.md` without redesign, then exact-head CI, physical iPhone validation and Independent QA.** Phase 5 Search remains planned but is not active. Preserve frozen Guide mechanics, PR #74 programme windowing and the provider-independent domain boundary. Physical Android interaction validation remains deferred until Android hardware is available.