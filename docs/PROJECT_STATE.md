...
Current implementation priority: **PR #149 / issue #148 — Acc Design Refinement blockers #5800174670 and #5800351843 are implemented: base-density Kijktip titles are capped to two lines while Larger Text remains uncapped, and development-only discovery states now deterministically expose Series ≥12 plus all discovery modules; exact-head CI and focused physical iPhone re-convergence are the next gates before Independent QA/final Lead merge acceptance**
...
6. **Phase 5 — Search and Discovery:** **ACTIVE**. Kijktip is fully closed and **Phase 5A — Guide Search is CLOSED**. Vanavond has a canonical product contract in `docs/TONIGHT_PRODUCT_DEFINITION.md`; its empirical classification gate is complete in `docs/TONIGHT_CLASSIFICATION_RESEARCH_2026-09-23.md`; Series scope is frozen as general/mainstream scripted episodic content; and the owner-approved production visual/specification is merged and canonical in `design/current/TONIGHT.md` + `docs/TONIGHT_VISUAL_CONVERGENCE.md` via PR #141. PR #144 / issue #142 has now delivered, merged, deployed and live-verified the required central provider-independent classification/enrichment foundation: ingest-owned sibling semantics, atomic schedule lifecycle, full-provider evidence preservation and a bounded separate semantic read boundary. Both Lead certainty blockers are closed and Independent QA passed. Hosted migration/Edge deployment, authoritative backfill, exact classification completeness, zero-orphan checks and public bounded-read smoke are recorded in `docs/TONIGHT_CLASSIFICATION_DEPLOYMENT_2026-09-23.md`. The next implementation step is the first production Vanavond runtime; raw provider taxonomy/title heuristics in UI remain forbidden.
...
## Vanavond first production runtime — PR #149 Development-complete candidate
...
**Next gate:** exact-head CI for these focused convergence corrections → return the new exact head to Acc Design Refinement / physical iPhone convergence → Development corrections only if new physical evidence requires them → Independent QA → final Technical Lead merge gate. PR #149 must not merge/deploy before those gates.
...
- Vanavond runtime implementation after accepted design/spec merge and central Film/Series/Sport classification-enrichment foundation;
- physical Android validation.

## Current next step
**Execute issue #142 — Vanavond central programme classification enrichment foundation.** Implement one central provider-independent sibling classification keyed to canonical broadcast identity, using structured provider evidence without leaking provider vocabulary to mobile. Film / general-mainstream Series / Sport-event-or-highlights semantics must fail closed when unknown. Preserve tri-state live/repeat semantics, safe Programme.id correction lifecycle and Guide performance. No production Film/Series/Sport carousel population before this foundation is merged and, if hosted schema/runtime changes are introduced, deployed/live-verified.

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/TONIGHT_PRODUCT_DEFINITION.md`, `docs/TONIGHT_CLASSIFICATION_RESEARCH_2026-09-23.md`, `docs/TONIGHT_VISUAL_CONVERGENCE.md`, `design/current/TONIGHT.md`, `docs/SEARCH_PRODUCT_DEFINITION.md`, `docs/PRODUCT.md`, `docs/UX.md`, `docs/ARCHITECTURE.md`, `docs/DATA.md`, `docs/ENGINEERING_QUALITY_POLICY.md` and `docs/BUILD_SPEC.md` before changing the repository. Phase 4, Kijktip and Phase 5A Guide Search are CLOSED. Vanavond product semantics, Series scope and production visual design are owner-approved, merged and canonical. Issue #142 is the active engineering gate. Film/Series/Sport runtime population remains blocked on central provider-independent classification enrichment. Preserve the 06:00 television-day model, accepted Guide/Search architecture and provider-independent domain. Do not introduce direct mobile TMDB or raw-provider category logic. Physical Android interaction validation remains deferred until Android hardware is available.
