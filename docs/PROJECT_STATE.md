...
## Phase status
1. **Phase 1A — Totaal interaction/technical baseline:** complete and physically accepted on iPhone. The final Totaal production visual design was subsequently owner-approved on 2026-09-21; production visual convergence remains to be specified and implemented in Phase 4.
...
For Totaal, the **production visual design is owner-approved**. Canonical composition is `/Teevee/Totaal production design - light dark expanded condensed.png` (`file_00000000aafc8230b3218e6577a1d912`) and the written contract is `design/current/guide/TOTAAL.md`. Frozen interaction/data constraints remain intact: 2D time/channel navigation, real programme geometry, shared day selector + `Nu`, native inertia/bounce/directional lock, Programme Detail context, Instrument Sans and the proven performance/windowing architecture. `docs/TOTAAL_VISUAL_CONVERGENCE.md` is still an inactive stale pre-approval draft and must be rewritten/reactivated against the accepted visual before Development begins.
...
## EXACT NEXT STEP
**Write and owner-review the Totaal production visual-convergence specification from the accepted 21 September design. Start from `design/current/guide/TOTAAL.md`, `docs/VISUAL_BASELINE.md`, the exact canonical Library asset `file_00000000aafc8230b3218e6577a1d912`, the accepted shared day-selector contract and the frozen Totaal interaction/performance architecture. Reconcile and reactivate `docs/TOTAAL_VISUAL_CONVERGENCE.md` with exact production metrics, responsive/Dynamic-Type rules, edge cases and acceptance criteria. Do not change runtime code until that specification is owner-approved and merged. Afterward Development performs Totaal visual convergence, then exact-head CI, physical iPhone validation and Independent QA. Phase 5 Search remains planned but is not the active next step.**

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/PRODUCT.md`, `docs/UX.md`, `docs/VISUAL_BASELINE.md`, `design/current/guide/TOTAAL.md`, `docs/ARCHITECTURE.md`, `docs/DATA.md`, `docs/ENGINEERING_QUALITY_POLICY.md` and the relevant ADRs before changing the repository. Phase 4 remains ACTIVE. Per-zender, Programme Detail and Nu & Straks production convergence are merged and accepted on iPhone. Totaal interaction mechanics, television-day semantics and performance hardening are proven, and **Totaal final production visual design is now owner-approved** in the exact canonical Library asset `file_00000000aafc8230b3218e6577a1d912`. The next step is to rewrite/reactivate `docs/TOTAAL_VISUAL_CONVERGENCE.md` into the owner-reviewed production specification before any runtime visual code changes. Phase 5 Search is planned but not active. Preserve frozen Guide mechanics and the provider-independent domain boundary. Physical Android interaction validation remains deferred until Android hardware is available.