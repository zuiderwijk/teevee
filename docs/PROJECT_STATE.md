...
Status: ACTIVE — **Phase 6 — Personal Features definition, with targeted production data-integrity correction in review**.
Current implementation priority: **Issue #151 / PR #152 — review the D0 non-destructive classification sibling recovery before any hosted migration/deploy/recovery; preserve ADR-0007 schedule authority and Vanavond fail-closed semantics**
...
## Vanavond D0 classification bootstrap correction — ISSUE #151 / PR #152 IN REVIEW

Post-acceptance physical Vanavond → Live validation exposed a hosted **classification completeness** gap in the active 23 September television day, not a UI or classifier-semantic regression. Read-only hosted evidence before the correction shows D0 at **513 canonical programmes / 5 classification siblings**, with **136** evening candidates but only **5** classified and **0 / 0 / 0** eligible Film / Series / Sport. D+1 is healthy at **520 / 520** full-day classification and **142 / 142** evening classification, yielding **6 / 14 / 1** eligible Film / Series / Sport. Orphan classifications remain **0**.

The root cause is bootstrap ownership: D0 canonical coverage was generated at `2026-09-23T00:17:01.240Z`, before classification deployment; the post-deploy guide-horizon refresh correctly skipped later D0 provider data as partial while authoritatively storing source-complete D+1…D+5. The five D0 siblings were generated at the D+1 refresh timestamp and belong to broadcasts intersecting that boundary.

PR #152 establishes the durable distinction **authoritative schedule replacement completeness != enrichment recovery eligibility**. Normal `ingestProviderSchedule` and ADR-0007 remain unchanged: partial provider coverage is never schedule authority. A separate protected bounded recovery path may reuse the same provider adapter/normaliser/central classifier to attach a sibling only when the currently stored canonical broadcast still exactly matches regenerated `id + channelId + startAt + endAt + title`. Recovery excludes normalisation-error channels, deterministically collapses exact duplicate source rows and refuses overlapping provider candidates; the service-role RPC independently rejects overlapping payloads. The persistence RPC takes the same per-channel advisory locks as schedule replacement, ignores observations older than newer coverage/classification state, and mutates only `teevee.programme_classifications`. Missing/corrected/ambiguous rows remain fail-closed; later authoritative replacement/rekey/deletion retains cascade ownership.

Deterministic offline tests cover partial-window schedule preservation, retained programmes outside the provider returnset, Film/Series/Sport/unknown semantics, missing evidence, start correction/rekey, stable-provider-id corrections, same-title repeats, duplicate/overlap ambiguity, channel normalisation errors, stale recovery, idempotency, zero-orphan lifecycle, raw-provider-field isolation and unchanged authoritative ingest. Latest disposable PostgreSQL 17 smoke run `35919038820`, job `107377942252`, executed the real base/classification/recovery migrations plus lifecycle assertions and completed **SUCCESS** with `BEGIN → DO → ROLLBACK`. The temporary workflow is removed from the final diff.

**No hosted migration, Edge deployment or production D0 recovery has been executed.** Exact pre/post hosted procedure and historical baseline are recorded in `docs/TONIGHT_CLASSIFICATION_D0_RECOVERY_2026-09-23.md`. Next gate is Technical Lead review of the final exact PR head; only after approval may merge/deploy/recovery be considered.
