...
Current implementation priority: **Issue #175 / PR #176 — reconcile and validate the owner-approved 49-channel canonical Guide expansion on top of merged channel personalisation.** PR #173 is merged as `a23c5a434f237859b06112217433a12955cfa725` after exact-head CI #1454, owner physical iPhone PASS and Independent QA PASS.
...
- Accepted durable architecture contracts are ADR 0001 through ADR 0013; ADR 0010 is canonical for the classification sibling boundary, ADR 0011 for broadcast-keyed external-content identity, ADR 0012 for durable bounded EPG refresh orchestration and ADR 0013 for the local canonical channel-personalisation projection.
- **Active 49-channel expansion:** Issue #175 / PR #176 implements the exact owner-approved 49-channel set/order, explicit 36 NL + 13 BE provider ownership, 12-day bounded multi-source refresh planning, deterministic 12→49 personalisation reconciliation and local canonical logo ownership. Physical refinement adds 17 centrally resolved dark-background variants and a `__DEV__`-only `npm run start:physical49` Guide/Search transport that exercises the real client contracts without production activation. Source-specific hosted capacity for group size 12 is measured PASS across all five NL/BE groups; evidence: `docs/CHANNEL_EXPANSION_CAPACITY_SMOKE_2026-09-25.md`. Production activation remains gated by exact-head CI, focused physical iPhone re-test, Independent QA and the existing release/deployment policy.
...
- **Merged channel-personalisation foundation:** Issue #170 / PR #173 is canonical. Exact candidate `ee482e9de279093b29fc02bb07e5fc5cd099ad36` passed owner physical iPhone stress validation and Independent QA, with exact-head CI #1454 green, and merged as `a23c5a434f237859b06112217433a12955cfa725`. Mijn zenders retains the accepted two-zone model, handle-only drag with fixed native gesture owner, immediate visibility persistence, local management Search, accessibility one-step reorder actions and shared Guide projection. Search remains full-catalog and hidden-channel navigation remains transient until explicit add. Contract: `docs/CHANNEL_PERSONALISATION.md`; ADR 0013 is canonical.
