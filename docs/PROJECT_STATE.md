...
The durable run envelope is **1024 work items**. The owner-approved worst case of 49 channels × 12 television days × group size one is **588 jobs**, now covered directly in both planner tests and the executable SQL lifecycle. Current NL capacity may still use all 12 channels in one group because that exact shape is production-measured; group size remains source-owned configuration. Source validation requires complete one-to-one canonical/provider mapping. The exact migration is behaviorally executed in normal CI on disposable **PostgreSQL 17**, including role boundaries, duplicate/stale attempts, lease recovery, retry exhaustion, distinct buckets, authority aggregation, Guide-before-TMDB ordering, failed-run suppression, deferred external-content unavailable/provider/persistence retries, retry-to-success, max-attempt staging cleanup, Guide-authority isolation and the 588-job envelope. PR #168 parser semantics, canonical Programme, classification and ADR 0007/0008/0010/0011 remain unchanged.
