...
- **Active Search/channel-personalisation increment:** Issue #170 / PR #173 implements one shared persisted Mijn-zenders selection/order over the complete supported catalog. Owner acceptance #5819111859 freezes the two-zone management UX from #5819090800. Physical iPhone review #5823162428 found one drag-termination blocker on candidate `42e93df...`: drag cleanup depended on Reanimated spring completion and could strand overlay/gap/disabled scrolling. Development now owns terminal teardown synchronously in a tokenized drag lifecycle; changed-drop persists once, no-op/cancel never persist, every RNGH finalize has a safe terminal path, and stale animation completion cannot affect a newer session. Focused physical iPhone re-test remains required. Search remains full-catalog and is the recovery/education route for hidden channels and deterministic channel-management intents. Contract: `docs/CHANNEL_PERSONALISATION.md`; evidence: `docs/TVGIDS_APP_SEARCH_CHANNEL_EVIDENCE_2026-09-24.md`. Phase 5A remains closed; the exact 49-channel production activation is a separate follow-up.
