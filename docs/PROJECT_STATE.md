...
## EXACT NEXT STEP
**Converge the accepted Programme Detail direction into an exact production implementation specification before new runtime work. Start from `design/current/PROGRAMME_DETAIL.md`, `docs/UX.md`, `docs/DESIGN_SYSTEM.md` and the current implementation. Specify `Herinner mij` + `Bewaar`, the contextual sticky bottom copies and their show/hide hysteresis, historical/current/future broadcast states, larger-system-text stacking, accessibility, light/dark/system, persistence/action ownership boundaries and Programme Detail round-trip behaviour. Do not invent reminder/saved-programme backend architecture or notification semantics beyond the accepted current phase; escalate any missing product decision that materially affects a durable contract. After the specification is canonical, implement it as a separate Phase 4 increment with the normal risk-based CI, physical validation and Independent QA gates.**

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/ENGINEERING_QUALITY_POLICY.md`, ADR 0007, ADR 0008 and the relevant accepted visual handoff before changing the repository. Phase 1A, Phase 1B, Phase 2 and Phase 3 are closed on iPhone. Phase 4 is active. Per-zender production convergence is merged and closed through PR #88/#89: preserve its 52/176 fixed rows, 4/52/24 expanded temporal composition, 0/52/0 condensed composition, semantic anchors, channel identity stability, safe-area handling, selected-channel schedule-height ownership, gestures, D-2..D+7 and the frozen PR #81 fixed-native-viewport/collapse-isolation architecture. The next canonical handoff task is Programme Detail production specification from the accepted `design/current/PROGRAMME_DETAIL.md` direction before any new Programme Detail runtime increment.