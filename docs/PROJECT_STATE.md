...
Search is paused until the Kijktip data/enrichment path, approved Guide presentation wiring, deterministic verification and required acceptance gates are complete. **PR #126 is merged as `f5bc682f27848bbe6ca6adaa3116dfc6e60d3f6d`** and completes the production backend/data/runtime foundation: server-side `tips.rss` ingestion, deterministic Tier B/C matching, private persisted `ProgrammeEditorialSignal` snapshots, independent protected editorial refresh, fail-open typed hosted transport, current/non-current selected-day enrichment ownership and separate mobile runtime editorial state. Canonical `Programme` remains provider/enrichment-independent. **PR #127 is the active visible Guide-presentation increment:** it wires those existing signals into the already owner-approved Per-zender (PR #122) and Nu & Straks (PR #123) presentations only; Totaal and Programme Detail remain unchanged. PR #127 must pass exact-head Lead review and the applicable physical/accessibility acceptance gates before Kijktip is considered complete. After full Kijktip acceptance, resume Phase 5 with Search. Tonight remains deferred/provisional.
...
## Current next step
**Complete PR #127 — visible Kijktip Guide presentation — without reopening the merged #126 data/runtime architecture.** Per zender must render only the selected-window `ProgrammeEditorialSignal[]`; Nu & Straks must render only the shared current-day runtime signals and react to editorial-only updates through the existing subscription boundary. Preserve the owner-approved PR #122/#123 geometry, Dynamic Type and single-programme accessibility targets; keep Totaal, Programme Detail and canonical `Programme` unchanged. Return the exact implementation head to Lead, then complete the required physical/accessibility acceptance gates. Do not start Search until the complete Kijktip vertical slice is accepted. Tonight remains deferred/provisional.
