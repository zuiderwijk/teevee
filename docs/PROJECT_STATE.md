...
Current implementation priority: **Issue #170 / PR #173 — Channel personalisation: one persisted `Mijn zenders` selection/order shared by all Guide presentations, with full-catalog Search recovery/education. Issue #167 / merged PR #172 is production-closed after the independent normal six-hour cron proved the bounded v13 path end-to-end.**
...
- Accepted durable architecture contracts are ADR 0001 through ADR 0012; ADR 0010 is canonical for the classification sibling boundary, ADR 0011 for broadcast-keyed external-content identity and ADR 0012 for durable bounded EPG refresh orchestration. PR #173 proposes ADR 0013 for the local canonical channel-personalisation projection; it becomes canonical on merge.
...
- **Active Search/channel-personalisation increment:** Issue #170 / PR #173 implements one shared persisted Mijn-zenders selection/order over the complete supported catalog. Owner acceptance #5819111859 freezes the two-zone management UX from #5819090800. Physical iPhone review #5823162428 found a drag blocker on candidate `42e93df...`. The first correction made terminal teardown synchronous, but focused iPhone re-test exposed the deeper pickup failure: setting drag state replaced the active `ChannelManagementRow` with a drop-slot view, unmounting the GestureDetector that owned the live Pan. The active row/handle now remains mounted for the entire drag and only its sighted content becomes an invisible placeholder/insertion target; the same gesture object therefore continues to own updates/finalize. A second focused iPhone recording then exposed an iOS-only directional cancellation: downward provisional reordering worked, but reversing upward moved the active gesture-owner row to an earlier native sibling index and immediately cancelled it. The source row is therefore now also fixed at its original native sibling/layout slot for the entire gesture; only intervening neighbour rows animate by deterministic translateY offsets and the insertion line moves relative to the fixed source. RNGH `onFinalize` remains the sole terminal gesture callback and maps success to drop / failure-cancel to restore. Tokenized teardown remains synchronous and independent from the visual spring. Focused physical iPhone re-test remains required. Search remains full-catalog and is the recovery/education route for hidden channels and deterministic channel-management intents. Contract: `docs/CHANNEL_PERSONALISATION.md`; evidence: `docs/TVGIDS_APP_SEARCH_CHANNEL_EVIDENCE_2026-09-24.md`. Phase 5A remains closed; the exact 49-channel production activation is a separate follow-up.
...
Hosted persistence is deployed from reviewed migration blob `3cd8bbc2d0e6a1a7bb4320d0bc4510f0b543650e`; Supabase recorded remote migration `20260924001500_create_programme_external_content_reference`. `teevee.programme_external_content_references` is live with RLS enabled, service-role-only intended authority, SECURITY INVOKER/empty-search-path writers, exact-broadcast ownership triggers and 0 dangling rows at rollout. `epg-refresh` v13 is ACTIVE with the existing custom auth boundary after the merged PR #172 orchestration rollout; critical orchestration files were verified against merge commit `f66c396...` during deployment. Full rollout evidence: `docs/EXTERNAL_CONTENT_IDENTITY_DEPLOYMENT_2026-09-24.md`.
...
Issue #167 exposed two separate hosted CPU ownership failures. PR #168, merged as `6a9b601ae7e2cac8aa0c2c94302c5aa16411a76f`, fixed feed-wide pre-filter materialisation with the bounded streaming XMLTV parser: the narrow RTL4 production path dropped from ~2.16 s / ~182 MB to ~1.28 s CPU / ~12–14 MB. A later v12 full-horizon smoke still hit the hard 2000 ms CPU ceiling after already committing five complete television days, proving that the remaining problem was ownership of D-3..D+8 by one Edge invocation rather than parser breadth.

PR #172 is **merged and production-closed** as `f66c3967d81a76113e8c29a0e9e6ffb58bae105d`; exact-main CI #1375 / run `36023646968` passed. ADR 0012 is canonical. The reviewed orchestration migration is registered under canonical version `20260924095903_create_epg_refresh_orchestration`, and hosted `epg-refresh` **v13** is ACTIVE. The durable Guide unit is **provider source/feed × Amsterdam television-day × bounded channel group**. Postgres owns run/job identity, one common observation timestamp, exact source/day/group scope, attempts, leases, retries and terminal authority. Exact-set stored results can succeed directly; `ignored-stale` still requires atomic canonical coverage proof under the ADR-0007 channel locks. Provider-partial, unattributed/no-safe or incomplete canonical scope remains terminal `incomplete`, never false success.

ADR 0011 sequencing is preserved across the multi-invocation horizon. Guide children never call TMDB. A safely stored current/future Film/Series observation may stage private identity evidence, but external-content work is not dispatched until all Guide jobs are terminal. External-content operational/provider/persistence failures use their own bounded retry lifecycle and never roll back Guide authority; terminal success clears staging.

The production rollout proved the architecture twice. The protected manual run created exactly 12 Guide jobs and completed without HTTP 546/resource kills. The first independent normal six-hour cron then ran at **2026-09-24 18:17 UTC** (pg_cron run 208), created durable run 2 with 12 jobs, reached Guide terminal at 18:17:33Z and external-content terminal at 18:17:47Z, with **5 succeeded / 7 expected provider-partial incomplete / 0 failed** Guide jobs. Across that scheduled v13 execution there were **0 HTTP 546 responses, 0 CPUTime shutdowns, 18 normal EarlyDrop shutdowns, max observed CPU 1282 ms and max observed memory 15,553,639 bytes (~14.8 MiB)**. External-content completed 5/5 applicable jobs with 0 failures, staging remaining 0, 160 references and 0 dangling references. Issue #167 is closed as completed; the remaining D-2..D+7 gap is solely the already-documented temporary development-provider horizon limitation and stays a separate production-provider/release gate.

The durable run envelope remains **1024 work items**. The owner-approved worst case of 49 channels × 12 television days × group size one is **588 jobs**, covered by planner and executable PostgreSQL-17 lifecycle tests. Current NL operation may still use all 12 channels in one group because that exact one-day shape is production-measured; group size remains source-owned configuration. PR #168 parser semantics, canonical Programme, classification and ADR 0007/0008/0010/0011 remain unchanged.
...
## Current next step
**Complete the active issue #170 / PR #173 Mijn-zenders implementation through focused physical iPhone convergence, Independent QA and the final merge gate.** Issue #167 / PR #172 is production-closed. Keep the exact 49-channel production activation as the separate next data/provider increment; do not fold provider mapping/EPG expansion into PR #173.

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/PROGRAMME_EXTERNAL_IDENTITY_SOURCE_RESEARCH_2026-09-23.md`, `docs/TMDB_MATCHING_RESEARCH_2026-09-23.md`, `docs/TONIGHT_PRODUCT_DEFINITION.md`, `docs/TONIGHT_VISUAL_CONVERGENCE.md`, `design/current/TONIGHT.md`, `docs/SEARCH_PRODUCT_DEFINITION.md`, `docs/PRODUCT.md`, `docs/UX.md`, `docs/ARCHITECTURE.md`, `docs/DATA.md`, `docs/ENGINEERING_QUALITY_POLICY.md` and `docs/BUILD_SPEC.md` before changing the repository. Phase 4 and Phase 5 are CLOSED. Current owner priority is issue #170 / PR #173 channel personalisation; the Premium Artwork & Content Identity foundation remains canonical and the exact 49-channel production activation stays a separate follow-up. Preserve the accepted Guide/Search/Vanavond architecture, the 06:00 television-day model, provider-independent domain boundaries and the existing ProgrammePersonalState authority. Do not introduce direct mobile TMDB/raw-provider category logic, broad content-catalogue abstractions or duplicate save/reminder persistence. Physical Android interaction validation remains deferred until Android hardware is available.
