...
Status: ACTIVE — **Phase 5 — Vanavond production-design handoff**.
Current implementation priority: **merge the owner-approved Vanavond production visual/specification, then design/implement the central provider-independent Film/Series/Sport classification-enrichment boundary before production category runtime; the Vanavond UI must not compensate with raw-provider heuristics**
...
6. **Phase 5 — Search and Discovery:** **ACTIVE**. Kijktip is fully closed and **Phase 5A — Guide Search is CLOSED**. Vanavond has a canonical product contract in `docs/TONIGHT_PRODUCT_DEFINITION.md`; its empirical classification gate is complete in `docs/TONIGHT_CLASSIFICATION_RESEARCH_2026-09-23.md`; Series scope is frozen as general/mainstream scripted episodic content; and the owner-approved production visual/specification now lives in `design/current/TONIGHT.md` + `docs/TONIGHT_VISUAL_CONVERGENCE.md` on the current design PR. Film, Series and Sport still require **central provider-independent classification mapping/enrichment** before production category implementation. The next development gate after the design PR merges is that classification/enrichment foundation, not raw Vanavond UI heuristics.
...
## Current next step
**Lead-review and merge the owner-approved Vanavond docs/design handoff.** The accepted visual is `/Teevee/Vanavond production design - hoofdvariant.png` (`file_00000000c6e081f4b4e0ed5e08a9c1fe`); `design/current/TONIGHT.md` and `docs/TONIGHT_VISUAL_CONVERGENCE.md` are the implementation authority on merge. After that merge, the next development increment is the central provider-independent Film/Series/Sport classification/enrichment foundation required by the empirical research. Do not start production category UI by mapping current provider genres directly. Artwork remains optional; the desired TMDB no-language poster direction requires a separate central enrichment/rights/provenance path and never a direct mobile dependency.

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/TONIGHT_PRODUCT_DEFINITION.md`, `docs/TONIGHT_CLASSIFICATION_RESEARCH_2026-09-23.md`, `docs/TONIGHT_VISUAL_CONVERGENCE.md`, `design/current/TONIGHT.md`, `docs/SEARCH_PRODUCT_DEFINITION.md`, `docs/PRODUCT.md`, `docs/UX.md`, `docs/ARCHITECTURE.md`, `docs/DATA.md`, `docs/ENGINEERING_QUALITY_POLICY.md` and `docs/BUILD_SPEC.md` before changing the repository. Phase 4, Kijktip and Phase 5A Guide Search are CLOSED. Vanavond product semantics, Series scope and production visual design are owner-approved; the docs/design handoff must merge before implementation authority moves to `main`. Film/Series/Sport runtime population remains blocked on central provider-independent classification enrichment. Preserve the 06:00 television-day model, accepted Guide/Search architecture and provider-independent domain. Do not introduce direct mobile TMDB or raw-provider category logic. Physical Android interaction validation remains deferred until Android hardware is available.
