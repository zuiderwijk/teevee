...
PR #172 is the active server-only orchestration correction. Proposed ADR 0012 defines the candidate durable unit of work as **provider source/feed × Amsterdam television-day × bounded channel group**. `guide-horizon` becomes a lightweight run planner; Postgres owns run/job identity, one common observation timestamp, source/day/group scope, attempts, retry state and durable outcomes. Durable success is now exact-scope: only `stored` or `ignored-stale` for the complete expected canonical child set is `succeeded`; provider-partial, unattributed/no-safe input or a channel-local blocked subset is terminal `incomplete`, and parent state exposes `incomplete` instead of falsely reporting `completed`. Distinct six-hour request keys are always persisted rather than coalesced into an older active run, while Vault/dispatch-unavailable reasons are durable on the queued run/job. Child Edge requests carry only opaque job/attempt identity and claim their real scope server-side. Current concurrency is intentionally one child globally; the **8-minute lease exceeds Supabase's current 400-second paid hosted worker maximum**, so lease recovery cannot overlap a still-live old worker. The present NL source may use all 12 current channels in one group because that exact shape is production-measured, but group size is an operational source setting rather than a product constant. Source validation also requires every source-owned canonical channel to have exactly one provider mapping, so the queued 49-channel NL+BE expansion cannot silently omit a channel. The exact orchestration migration is behaviorally exercised in CI on disposable PostgreSQL 16, including duplicate/stale attempts, lease recovery, retry exhaustion, sibling continuation, parent aggregation, distinct buckets, common observation ownership and role rejection. PR #168 parser semantics, canonical Programme, classification, ADR 0007/0008/0011 and the 20 s TMDB budget remain unchanged.
