...
- Relevant durable architecture contracts are ADR 0001 through ADR 0012; ADR 0010 is canonical for the classification sibling boundary, ADR 0011 for broadcast-keyed external-content identity, and ADR 0012 for bounded durable EPG refresh orchestration.
...
PR #172 is the active server-only orchestration correction. ADR 0012 defines the durable unit of work as **provider source/feed × Amsterdam television-day × bounded channel group**. `guide-horizon` becomes a lightweight run planner; Postgres owns run/job identity, one common observation timestamp, source/day/group scope, leases, attempts, retry state and durable outcomes. Child Edge requests carry only opaque job/attempt identity, claim their real scope server-side and execute one normal bounded ingest + fail-open TMDB enrichment. Current concurrency is intentionally one child globally; the present NL source may use all 12 current channels in one group because that exact shape is production-measured, but group size is an operational source setting rather than a product constant. The queued 49-channel expansion can therefore add/split NL/BE source groups without redesigning freshness, auth, stale-write or retry ownership. PR #168 parser semantics, canonical Programme, classification, ADR 0007/0008/0011 and the 20 s TMDB budget remain unchanged.
