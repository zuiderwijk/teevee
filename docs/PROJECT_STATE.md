...
- following programme targets remain directly adjacent, non-overlapping minimum **44 pt iOS / 48 dp Android**; visible content is vertically centred in all three targets;
...
## EXACT NEXT STEP
**Reconcile the existing runtime PR #96 against the owner-accepted PR #102 final Nu & Straks physical-composition baseline now canonical on `main`. Preserve every already-accepted PR #96/PR #100 contract that PR #102 does not supersede. Implement only the final deltas: remove all standalone visible reference-time copy and the obsolete 88-pt/two-lane reference-time context so Nu & Straks uses a 52-pt utility-only Primetime/Nu row + 52-pt rail = 104-pt persistent stack at every font scale; render the rail-bottom line as exactly 1 pt semantic `railTick` at opacity 0.78 to match quarter-hour ticks; keep the reference programme minimum 64 pt but bottom-align its title with 0 pt extra bottom inset; set dedicated reference→following gap to 0 pt; move the former 4 pt into channel bottom padding (12 pt total) so normal channel rows remain 216 pt iOS / 228 dp Android; centre visible content in all three following targets while retaining three adjacent non-overlapping 44/48-safe Pressables. Preserve the physically accepted Larger Text following composition, dark mode, 15-minute rail/ticks, Nu/Primetime, vertical channel-position preservation, native rail momentum/no-secondary-scrollTo, Programme Detail round-trip, unavailable-data catalogue preservation, deferred module loading and Expo Router app-tree guard. Require deterministic coverage, exact-head CI and Lead re-review. Physical acceptance remains PAUSED until a new PR #96 exact head is approved for a focused physical iPhone revalidation; Independent QA remains after physical PASS.**

Owner checkout: `~/projects/teevee`.

## Resume instruction
> Read `AGENTS.md`, this file, `docs/ENGINEERING_QUALITY_POLICY.md`, ADR 0007, ADR 0008 and the relevant accepted visual handoff before changing the repository. Phase 1A, Phase 1B, Phase 2 and Phase 3 are closed on iPhone. Phase 4 is active. Per-zender production convergence is merged and closed through PR #88/#89. Programme Detail production convergence is merged and closed through PR #92. Nu & Straks production design is canonical through PR #93, PR #97, PR #100 and owner-approved final physical refinement PR #102. Runtime PR #96 exact head `224e7df1d47dbb5c42694cb36469de673496e7d5` passed dark mode, Larger Text, vertical-position preservation, Nu, Primetime and 15-minute rail/tick physical checks but was not fully accepted because visible reference time, rail-bottom treatment and reference→following rhythm still required refinement. Reconcile the existing PR #96 to current `docs/NU_EN_STRAKS_VISUAL_CONVERGENCE.md`; do not resume physical acceptance, request Independent QA or merge until the new runtime head passes exact-head CI and Lead re-review.